...
const SidebarItem = ({ icon, name }: { icon: React.ReactNode, name: string }) => {
  return (
    <div className="sidebar-item group relative flex items-center justify-center my-2 cursor-pointer">
      ...
      <div className="sidebar-tooltip group-hover:scale-100 absolute left-16 ml-2 p-2 rounded bg-primary text-white text-sm opacity-0 group-hover:opacity-100 transform transition-all origin-left scale-0">{name}</div>
    </div>
  )
}
